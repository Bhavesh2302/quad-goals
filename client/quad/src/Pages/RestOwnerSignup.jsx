...
const RestOwnerSignup = () => {
  ...
  return (
    <Box>
      {!show ? (
        <RestOwnerSignupComp show={show} setShow={setShow} />
      ) : (
        <RestOwnerLoginComp show={show} setShow={setShow} />
      )}
      <Box mb={"20px"}>
        <Button variant={"unstyled"} onClick={handleShowToggle}>
          {!show
            ? "Already Restaurant Owner? Login"
